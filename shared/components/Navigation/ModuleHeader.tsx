...
export function ModuleHeader({ title, subtitle, backLink, actions }: ModuleHeaderProps) {
  ...
  return (
    <div className="mb-10 pb-6 border-b border-secondary flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div>
        <div className="flex items-center gap-2 mb-2">
          <span className="w-1.5 h-1.5 rounded-full bg-primary"></span>
          <p className="text-[10px] uppercase tracking-[0.25em] font-bold text-muted-foreground">Quattro9 Studio</p>
        </div>
        <h2 className="text-3xl font-bold text-foreground tracking-tight leading-none">{sectionName}</h2>
      ...
      <div className="flex items-center gap-3">
        ...
        <button 
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-secondary border border-secondary rounded-lg text-foreground text-xs font-bold uppercase tracking-wider hover:bg-secondary hover:text-primary transition-all shadow-sm"
        >
