    ...
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-coffee tracking-tighter">Cartões de Crédito</h1>
            <p className="text-cappuccino text-xs font-bold uppercase tracking-widest mt-1">Gestão de Limites</p>
          </div>
        </div>
        <button 
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1,2].map(i => <div key={i} className="h-56 bg-stone-100 rounded-3xl animate-pulse" />)}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {cards.map(card => {
                ...
                <div className="relative z-10 flex flex-col justify-between h-full text-white">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                      <div 
                        className={`h-full rounded-full transition-all duration-500 ${progress > 80 ? 'bg-terracotta' : 'bg-olive'}`}
