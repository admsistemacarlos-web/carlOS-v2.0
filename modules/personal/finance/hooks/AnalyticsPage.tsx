...
import { 
  ...
  DollarSign, MapPin, ShoppingBag, AlertCircle, Calendar 
} from 'lucide-react';
import { useAnalyticsData, CHART_COLORS } from '../hooks/useAnalyticsData';
...
export default function AnalyticsPage() {
  ...
  const { loading, macroData, analyzeItemHistory } = useAnalyticsData();
  
  const [activeTab, setActiveTab] = useState<'categories' | 'items' | 'locations'>('categories');
  
  // States do Item Explorer
  ...
  // Efeito para limpar busca ao mudar de aba
  useEffect(() => {
    if (activeTab !== 'items') {
      setSearchTerm('');
      setItemData(null);
