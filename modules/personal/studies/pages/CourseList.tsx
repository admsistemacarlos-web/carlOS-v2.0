    ...
    <div className="space-y-8 animate-fade-in pb-20" onClick={() => setActiveMenuId(null)}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-foreground tracking-tighter">Biblioteca</h1>
            <p className="text-muted-foreground text-xs font-bold uppercase tracking-widest mt-1">Meus Cursos e Livros</p>
          </div>
        </div>
      ...
      {filteredCourses.length === 0 ? (
        <div className="text-center py-12 bg-card rounded-[2rem] border border-border shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCourses.map(course => (
            <div 
              key={course.id}
              className="group bg-card rounded-[2rem] border border-border p-6 cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all relative overflow-hidden"
            ...
            >
              ...
              <div className="absolute top-4 right-4 z-20">
                ...
                {activeMenuId === course.id && (
                  <div className="absolute right-0 mt-2 w-32 bg-card rounded-xl shadow-xl border border-border overflow-hidden animate-fade-in">
                    <button 
              ...
              <div className="w-12 h-12 bg-secondary rounded-2xl flex items-center justify-center text-muted-foreground mb-4 group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                <BookOpen size={24} />
              </div>

              {course.category && (
                <span className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground bg-secondary px-2 py-1 rounded-md">
                  {course.category}
                </span>
              )}

              <h3 className="font-bold text-lg text-foreground mt-3 mb-1 truncate pr-8">{course.title}</h3>
              ...
              <div className="mt-auto pt-4 border-t border-stone-50">
                ...
                <div className="h-1.5 w-full bg-secondary rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-700 ${course.progress === 100 ? 'bg-emerald-500' : 'bg-primary'}`} 
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-card w-full max-w-lg rounded-[2rem] shadow-2xl border border-border flex flex-col max-h-[90vh] overflow-hidden animate-fade-in">
             ...
             <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4">
                <div>
                  <label className="text-[10px] font-bold text-foreground uppercase tracking-widest ml-1">Título</label>
                  <input
                    value={formData.title}
                    onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                    placeholder="Ex: React Avançado"
                    className="w-full bg-secondary border border-border rounded-xl p-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-olive/10"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-secondary border border-border rounded-xl p-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-olive/10 resize-none"
                  />
                </div>
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Dados de Acesso Externo (Opcional)</p>

                <div>
                  <label className="text-[10px] font-bold text-muted-foreground uppercase ml-1">Link do Curso</label>
                  <input
                    type="url"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-bold text-muted-foreground uppercase ml-1">Email de Acesso</label>
                    <input
                      type="email"
