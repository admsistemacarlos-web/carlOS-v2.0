...
const CATEGORY_CONFIG: Record<LogCategory, { label: string; icon: React.ReactNode; color: string; bg: string; unitSuffix?: string }> = {
  ...
  measurement: { label: 'Peso', icon: <Scale size={18} />, color: 'text-muted-foreground', bg: 'bg-secondary', unitSuffix: 'kg' },
};
      ...
      <div className="max-w-7xl mx-auto px-8 pt-8">
        ...
        {activePet && stats && (
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              {stats.categoryStats ? (
                  <div className={`col-span-2 p-6 rounded-[2rem] border shadow-sm flex items-center justify-between transition-all ${getCategoryStyle(filterCategory as string).bg} border-opacity-50`}>
                      <div>
                          ...
                          <p className="text-4xl font-bold text-foreground tracking-tighter">R$ {stats.categoryStats.totalSpent.toFixed(2)}</p>
                      ...
                      <div className={`p-5 bg-card rounded-2xl shadow-sm ${getCategoryStyle(filterCategory as string).color}`}>
                  ...
                  <>
                      {/* Idade */}
                      <div className="bg-card p-6 rounded-[2rem] border border-border shadow-sm flex flex-col justify-between h-40">
                          <div className="flex justify-between items-start">
                              <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Idade</span>
                              <Calendar size={18} className="text-muted-foreground" />
                          </div>
                          <div>
                              <span className="text-3xl font-bold text-foreground tracking-tight">{stats.age.split(' ')[0]}</span>
                              <span className="text-sm text-muted-foreground ml-1 font-medium">{stats.age.split(' ')[1]}</span>
                          </div>
                      </div>
                      ...
                      <div className="bg-card p-6 rounded-[2rem] border border-border shadow-sm flex flex-col justify-between h-40">
                          <div className="flex justify-between items-start">
                              <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Peso</span>
                              <Scale size={18} className="text-muted-foreground" />
                          </div>
                          <div>
                              <span className="text-3xl font-bold text-foreground tracking-tight">{stats.weight}</span>
                              <span className="text-sm text-muted-foreground ml-1 font-medium">kg</span>
                          </div>
                      </div>
                  </>
              )}
              ...
              {/* Gasto do Período */}
              <div className="col-span-2 bg-primary/5 p-6 rounded-[2rem] border border-primary/10 flex flex-col justify-between h-40">
                 <div className="flex justify-between items-start">
              ...
              <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-9 gap-3">
                {(Object.keys(CATEGORY_CONFIG) as LogCategory[]).map(cat => (
                  <button
                    ...
                    className="flex flex-col items-center justify-center gap-2 bg-card border border-border p-4 rounded-2xl hover:border-primary/30 hover:shadow-md transition-all active:scale-95 group h-24"
                  >
                    ...
                    <span className="text-[9px] font-bold uppercase text-muted-foreground group-hover:text-foreground truncate w-full text-center tracking-wide">
            ...
            <div>
                  ...
                  <div className="flex gap-2 w-full md:w-auto">
                    <div className="relative flex-1 md:flex-none">
                        ...
                        <select
                            ...
                            className="w-full appearance-none bg-card border border-border text-muted-foreground text-[10px] font-bold uppercase tracking-wider py-2.5 pl-9 pr-8 rounded-xl focus:outline-none focus:border-primary/50 cursor-pointer shadow-sm"
                        >
              ...
              <div className="relative border-l-2 border-border ml-4 space-y-6 pb-10">
                  ...
                  return (
                    <React.Fragment key={log.id}>
                      <div className="relative pl-8 group animate-fade-in">
                          <div className={`absolute -left-[9px] top-4 w-4 h-4 rounded-full border-2 border-background shadow-sm z-10 ${config.bg}`}></div>
                          
                          <div className="bg-card p-5 rounded-2xl border border-border shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center hover:shadow-md transition-all gap-4">
                            <div className="flex-1">
                                <div className="flex items-center gap-2 mb-2 flex-wrap">
                                    ...
                                    {daysSince !== null && daysSince >= 0 && (
                                        <span className="text-[9px] text-muted-foreground font-bold bg-secondary px-2 py-0.5 rounded border border-border flex items-center gap-1">
                                            <Clock size={10} /> 
                                            {daysSince === 0 ? 'Hoje' : `Há ${daysSince} dias`}
                                        </span>
                                    )}
                                    {log.location && (
                                        <span className="text-[9px] text-muted-foreground font-bold flex items-center gap-1 bg-secondary px-2 py-0.5 rounded border border-border">
                                ...
                                <h4 className="font-bold text-foreground text-sm">
                                    {log.title}
                                    {log.value && suffix && <span className="ml-1 text-muted-foreground font-normal">({log.value} {suffix})</span>}
                                </h4>
                                ...
                                {log.next_due_date && (
                                    <div className="mt-2 flex items-center gap-2">
                                      <p className="text-[10px] text-primary flex items-center gap-1 font-bold uppercase tracking-wider">
                            ...
                            <div className="flex flex-row md:flex-col items-center md:items-end gap-3 md:gap-1 w-full md:w-auto justify-between md:justify-start">
                                {log.cost > 0 && (
                                    <div className="flex items-center gap-1.5" title="Lançado no Financeiro">
                                        <span className="text-base font-bold text-foreground">R$ {log.cost.toFixed(2)}</span>
                      ...
                      {intervalDays !== null && (
                          <div className="pl-8 pb-2 pt-2 relative -my-2 z-0">
                              <div className="flex items-center gap-2">
                                  <div className="text-[9px] font-bold text-muted-foreground bg-secondary border border-border px-2 py-0.5 rounded-full flex items-center gap-1 w-fit shadow-sm">
                ...
                {filteredLogs.length === 0 && (
                  <div className="pl-8 py-12 text-center">
                      <div className="bg-card border-2 border-dashed border-border rounded-2xl p-8">
                        <p className="text-muted-foreground text-sm font-medium">Nenhum registro encontrado para este período.</p>
                      </div>
                  </div>
                )}
              </div>
        ...
        {deleteConfirmationId && createPortal(
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in">
              <div className="bg-card w-full max-w-sm rounded-[2rem] p-6 shadow-2xl relative border border-border">
                  <div className="flex flex-col items-center text-center">
                      ...
                      <div className="flex gap-3 w-full">
                          <button 
                              ...
                              disabled={isDeleting}
                              className="flex-1 py-3 bg-secondary hover:bg-accent text-muted-foreground font-bold rounded-xl transition-colors text-xs uppercase tracking-widest"
                          >
                              Cancelar
                          </button>
                          <button 
