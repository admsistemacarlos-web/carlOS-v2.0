...
export default function PrayerLibraryPage() {
  ...
  const PrayerCard: React.FC<{ prayer: PrayerModel }> = ({ prayer }) => (
    <div 
        ...
        className="bg-white p-8 rounded-[2rem] border border-[#E6E2DE] shadow-sm hover:shadow-xl hover:border-[#D7CCC8] hover:-translate-y-1 transition-all group flex flex-col h-full cursor-pointer relative"
    >
        <div className="flex justify-between items-start mb-6">
        <div className="p-3 bg-stone-50 rounded-2xl text-[#8D6E63]">
            <Quote size={20} />
        </div>
        ...
        <h3 className="text-xl font-serif italic font-bold text-stone-800 mb-2 line-clamp-2 leading-tight">
        {prayer.title}
        </h3>
        ...
        <div className="flex-1 text-sm text-stone-500 leading-relaxed line-clamp-4 relative overflow-hidden font-serif">
        {prayer.content}
        <div className="absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-white to-transparent"></div>
        </div>

        <div className="mt-6 pt-4 border-t border-stone-50 flex justify-end">
            <span className="text-stone-400 text-[10px] font-bold uppercase tracking-widest flex items-center gap-1 group-hover:text-[#5D4037] transition-colors">
    ...
    <div className="w-full min-h-screen pb-20 animate-fade-in font-sans bg-[#FAF9F6]">
      ...
      <div className="px-8 pt-8 pb-6 flex flex-col md:flex-row justify-between items-start gap-4">
        <div>
          ...
          <p className="text-stone-500 text-sm mt-1">Modelos bíblicos e teológicos para inspiração.</p>
        </div>
      ...
      <div className="px-8 space-y-2">
        {loading ? (
          ...
          <div className="text-center py-20 bg-stone-50 rounded-[2rem] border border-dashed border-stone-200">
            <Quote className="mx-auto text-stone-300 mb-3" size={32} />
            <p className="text-stone-400">Sua biblioteca de orações está vazia.</p>
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-sm rounded-[1.5rem] shadow-2xl p-6 animate-fade-in border border-stone-200">
            <div className="flex flex-col items-center text-center">
              ...
              <h2 className="text-lg font-bold text-stone-800 mb-2">Excluir oração?</h2>
