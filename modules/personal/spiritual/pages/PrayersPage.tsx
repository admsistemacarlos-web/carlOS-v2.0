    ...
    <div className="w-full min-h-screen pb-20 animate-fade-in font-sans">
      ...
      <div className="px-8 grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="space-y-4">
            ...
            {loading ? <Loader2 className="animate-spin text-rose-500" /> : activePrayers.map(prayer => (
                <div key={prayer.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex gap-4 group">
                    <button 
                        onClick={() => markAsAnswered(prayer.id)}
                        className="w-6 h-6 mt-1 rounded-full border-2 border-slate-200 hover:border-rose-500 hover:bg-rose-50 flex items-center justify-center transition-all shrink-0"
                    ...
                    <div className="flex-1">
                        ...
                        {prayer.description && <p className="text-sm text-slate-500 mt-1">{prayer.description}</p>}
            ...
            {!loading && activePrayers.length === 0 && <p className="text-slate-400 text-sm italic">Sua lista de oração está vazia.</p>}
        </div>
        ...
        <div className="space-y-4">
            ...
            {answeredPrayers.map(prayer => (
                <div key={prayer.id} className="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100 flex gap-4 opacity-80 hover:opacity-100 transition-opacity">
                    ...
                    <div className="flex-1">
                        <h3 className="font-bold text-slate-700 line-through decoration-emerald-300">{prayer.title}</h3>
                        {prayer.description && <p className="text-sm text-slate-500 mt-1">{prayer.description}</p>}
      ...
      {isModalOpen && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl animate-fade-in p-6">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-lg font-bold text-slate-800">Novo Pedido</h2>
                <button onClick={() => setIsModalOpen(false)}><X size={20} className="text-slate-400" /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
                <input required autoFocus placeholder="Motivo da oração..." value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-rose-200 outline-none" />
                <textarea rows={4} placeholder="Detalhes (opcional)..." value={form.description} onChange={e => setForm({...form, description: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-rose-200 outline-none resize-none" />
