      ...
      <div className="bg-card p-6 rounded-[2rem] shadow-sm border border-border">
        ...
        <div className="flex items-center gap-4">
          <div className="relative flex-1">
            ...
            <input type="date" value={formData.startDate} onChange={handleDateChange} className="w-full pl-10 pr-4 py-3 bg-secondary rounded-xl text-foreground font-bold outline-none focus:ring-2 focus:ring-[#143d2d]/20" />
          </div>
        ...
        return (
          <div key={area} className="bg-card border border-border rounded-[2rem] overflow-hidden shadow-sm mb-8">
            ...
            <div className="p-6 md:p-8 space-y-8">
              ...
              <div><div className="flex justify-between items-end mb-3"><h4 className="text-sm font-bold text-foreground flex items-center gap-2"><Zap size={16} className="text-primary" /> Ações Fundamentais</h4><button onClick={() => addAction(area, 'recurring')} className="text-primary text-xs font-bold hover:underline">+ Adicionar</button></div><div className="space-y-2">{goal.actions.filter(a => a.type === 'recurring').map(action => (<div key={action.id} className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-primary"></div><input value={action.title} onChange={e => updateAction(area, action.id, 'title', e.target.value)} className="flex-1 bg-secondary rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-[#143d2d]/20" /><button onClick={() => removeAction(area, action.id)} className="text-muted-foreground hover:text-red-500"><Trash2 size={14} /></button></div>))}</div></div>
              <div><div className="flex justify-between items-end mb-3"><h4 className="text-sm font-bold text-foreground flex items-center gap-2"><Flag size={16} className="text-red-500" /> Peças Chave</h4><button onClick={() => addAction(area, 'fixed')} className="text-primary text-xs font-bold hover:underline">+ Adicionar</button></div><div className="space-y-2">{goal.actions.filter(a => a.type === 'fixed').map(action => (<div key={action.id} className="flex items-center gap-2"><select value={action.week} onChange={e => updateAction(area, action.id, 'week', Number(e.target.value))} className="bg-secondary rounded-lg px-2 py-2 text-xs font-bold text-muted-foreground outline-none cursor-pointer">{[...Array(12)].map((_, i) => <option key={i+1} value={i+1}>Sem {i+1}</option>)}</select><input value={action.title} onChange={e => updateAction(area, action.id, 'title', e.target.value)} className="flex-1 bg-secondary rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-[#143d2d]/20" /><button onClick={() => removeAction(area, action.id)} className="text-muted-foreground hover:text-red-500"><Trash2 size={14} /></button></div>))}</div></div>
            </div>
          </div>
        );
      })}
  ...
  const renderWizard = () => { 
    ...
    return (
        <div className="w-full min-h-screen bg-transparent pb-20 font-sans text-foreground">
            ...
            <div className="px-8 mb-8"><div className="flex justify-between max-w-md mx-auto relative"><div className="absolute top-1/2 left-0 w-full h-0.5 bg-accent -z-10 -translate-y-1/2"></div>{steps.map((s) => (<div key={s.num} className="flex flex-col items-center gap-2 bg-[#fafafa] px-2"><div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-500 ${step >= s.num ? 'bg-primary text-primary-foreground shadow-lg scale-110' : 'bg-accent text-muted-foreground'}`}>{s.num}</div><span className={`text-[10px] font-bold uppercase tracking-widest ${step >= s.num ? 'text-primary' : 'text-muted-foreground'}`}>{s.label}</span></div>))}</div></div>
            ...
            <div className="fixed bottom-0 left-0 w-full bg-card/90 backdrop-blur-md border-t border-border p-4 md:px-8 flex justify-between items-center z-50"><button onClick={() => setStep(prev => Math.max(1, prev - 1))} disabled={step === 1} className="px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-widest text-muted-foreground hover:bg-secondary disabled:opacity-30 transition-colors">Voltar</button>{step < 4 ? (<button onClick={() => setStep(prev => Math.min(4, prev + 1))} className="bg-primary hover:bg-[#0f2e22] text-white px-8 py-3 rounded-xl shadow-lg flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all active:scale-95">Próximo <ArrowRight size={14} /></button>) : (<button onClick={handleFinish} disabled={submitting} className="bg-primary hover:bg-[#0f2e22] text-white px-8 py-3 rounded-xl shadow-lg flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all active:scale-95 disabled:opacity-70">{submitting ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={14} />}{submitting ? 'Salvando...' : (editingCycleId ? 'Atualizar Planejamento' : 'Finalizar Planejamento')}</button>)}</div>
        </div>
    );
  };
