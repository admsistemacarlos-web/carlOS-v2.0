...
export default function ClientCredentialsVault({ clientId }: { clientId: string }) {
  ...
  return (
    <div className="bg-card p-6 rounded-lg border border-[#404040] shadow-sm">
      <div className="flex justify-between items-center mb-6">
        ...
        {!isAdding && (
          <button 
            ...
            className="text-[10px] font-bold text-[#737373] hover:text-[#D4D4D4] flex items-center gap-1 transition-colors bg-secondary px-2 py-1 rounded border border-[#404040]"
          >
            <Plus size={10} /> Add
          </button>
        )}
      ...
      {isAdding && (
        <form onSubmit={handleAdd} className="bg-secondary p-3 rounded-md mb-4 border border-[#404040] animate-fade-in">
          ...
          return (
            <div key={cred.id} className="group flex flex-col p-3 rounded-md bg-[#1A1A1A] border border-[#404040] hover:border-[#737373] transition-all gap-2">
              <div className="flex items-center gap-3">
                <div className="w-6 h-6 rounded bg-secondary flex items-center justify-center text-[#E09B6B] shrink-0">
                  <Lock size={12} />
                </div>
              ...
              {/* Campo de Senha */}
              <div className="bg-card px-2 py-1.5 rounded border border-[#404040] flex items-center justify-between gap-2">
