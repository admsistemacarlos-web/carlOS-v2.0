...
const SidebarItem: React.FC<{ to: string; icon: React.ReactNode; label: string; end?: boolean; onClick?: () => void }> = ({ to, icon, label, end, onClick }) => (
  <NavLink 
    ...
    onClick={onClick}
    className={({ isActive }) => `flex items-center gap-3 px-5 py-3 rounded-xl transition-all text-[11px] font-semibold tracking-tight ${isActive ? 'bg-white/5 text-white border border-white/10' : 'text-cappuccino hover:bg-white/5 hover:text-white'}`}
  >
    {icon}
    <span>{label}</span>
  </NavLink>
...
const PersonalLayout: React.FC<PersonalLayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-cream overflow-hidden font-sans personal-mode text-coffee">
      {/* Mobile Backdrop */}
      {isSidebarOpen && (
        <div 
          className="fixed inset-0 bg-coffee/20 backdrop-blur-sm z-40 lg:hidden" 
          onClick={closeSidebar}
        />
      )}

      {/* Sidebar - Minimalist Dark Context */}
      <aside className={`
        fixed inset-y-0 left-0 w-64 bg-deep-blue flex flex-col z-50 transition-transform duration-500 transform
        lg:translate-x-0 lg:static lg:inset-auto
        ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="p-8 pb-4">
          <div className="flex items-center justify-between mb-1">
            <h2 className="text-2xl font-semibold tracking-tighter text-white">carlOS</h2>
            <button 
              onClick={() => navigate('/professional')}
              className="p-1.5 rounded-lg bg-white/5 text-stone-400 hover:text-white hover:bg-white/10 transition-all"
              title="Ir para Profissional"
            >
              <Briefcase size={16} />
            </button>
          </div>
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-olive/80">Pessoal</p>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-1 py-4 overflow-y-auto">
          ...
          <div className="pt-8 pb-3 px-5">
             <p className="text-[9px] uppercase font-bold text-white/20 tracking-[0.3em]">Ambientes</p>
          </div>
          ...
          {isFinance && (
            <div className="mt-8 mx-2 p-4 bg-white/5 rounded-2xl border border-white/5 animate-fade-in">
              <p className="text-[9px] font-bold text-white/20 mb-4 tracking-[0.2em] uppercase px-2">Gestão</p>
              <div className="space-y-0.5">
                ...
                <NavLink 
                  ...
                  onClick={closeSidebar} 
                  className={({ isActive }) => `flex items-center gap-2 px-3 py-2 text-[10px] font-medium rounded-lg transition-colors ${isActive ? 'text-white bg-white/10' : 'text-cappuccino hover:text-white'}`}
                >
                  <PieChart size={14} />
                  Inteligência
                </NavLink>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="lg:hidden flex items-center justify-between p-6 bg-cream border-b border-stone-100">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="text-cappuccino hover:text-coffee transition-colors">
              <Menu size={24} />
            </button>
